...
inv_error_t MPU9250_DMP::lowPowerAccel(unsigned short rate) { return mpu_lp_accel_mode(rate); }
...
inv_error_t MPU9250_DMP::setLPF(unsigned short lpf) { return mpu_set_lpf(lpf); }
...
inv_error_t MPU9250_DMP::configureFifo(unsigned char sensors) { return mpu_configure_fifo(sensors); }

inv_error_t MPU9250_DMP::resetFifo(void) { return mpu_reset_fifo(); }
...
inv_error_t MPU9250_DMP::setSensors(unsigned char sensors) { return mpu_set_sensors(sensors); }
...
inv_error_t MPU9250_DMP::updateTemperature(void) { return mpu_get_temperature(&temperature, &time); }
...
inv_error_t MPU9250_DMP::dmpLoad(void) { return dmp_load_motion_driver_firmware(); }
...
bool MPU9250_DMP::tapAvailable(void) { return _tap_available; }
...
unsigned char MPU9250_DMP::dmpGetOrientation(void) { return mpu9250_orientation; }
...
inv_error_t MPU9250_DMP::dmpSetPedometerSteps(unsigned long steps) { return dmp_set_pedometer_step_count(steps); }
...
inv_error_t MPU9250_DMP::dmpSetPedometerTime(unsigned long time) { return dmp_set_pedometer_walk_time(time); }
...
static void orient_cb(unsigned char orient) { mpu9250_orientation = orient; }